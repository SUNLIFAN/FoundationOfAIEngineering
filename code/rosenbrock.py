...
def cal_dx(x, y):
    ...
    return -400*x*(-x**2 + y) + 2*x - 2 # 计算x偏导数

 

def cal_dy(x, y):

    return -200*x**2 + 200*y # 计算y偏导数
...
def train_grad(max_iter=100000, step=0.001): # 梯度下降迭代主函数
    ...
    while loss > 0.001 and iter_count < max_iter:
        ...
        print("iter_count: ", iter_count, "the loss:", "%.10f" % loss) # 每次迭代输出迭代序号和当前函数值
...
if __name__ == '__main__': # main主程序
    ...
    w = train_grad() # 调用梯度下降迭代函数
    print("[%.6f,"%w[0],"%.6f]" % w[1])# 显示w的值